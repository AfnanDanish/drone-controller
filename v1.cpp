...
void handleButtons() {
  ...
  // Rotary enable button
  rotaryEnabled = !digitalRead(ROTARY_ENABLE_PIN); // Inverted because of INPUT_PULLUP
}

void handleEncoder() {
  if (!rotaryEnabled) return; // Ignore encoder if not enabled
  
  int currentState = digitalRead(ROTARY_CLK_PIN);
  
  // If state changed, this means we had a pulse
  if (lastEncoderState != currentState && currentState == 1) {
    // If DT different from CLK, rotation is clockwise (CW), else counter-clockwise (CCW)
    if (digitalRead(ROTARY_DT_PIN) != currentState) {
      encoderValue++;
